...
export class CryptoService {
  ...
  public static async encrypt(
    ...
    try {
      ...
      const secretKey =
        await CryptoEngineFacade.Encryption.validateEncryptionPrerequisites(
          value,
          secretKeyVariable,
        );
      ...
      const { salt, iv, encryptionKey, hmacKey } =
        await CryptoEngineFacade.Encryption.generateEncryptionComponents(
          secretKey,
        );

      // Create encrypted payload using Web Crypto API
      const { raw } =
        await CryptoEngineFacade.Encryption.createEncryptedPayload(
          value,
          salt,
          iv,
          encryptionKey,
  ...
  public static async encryptMultiple(
    ...
    return Promise.all(
      values.map((value) => this.encrypt(value, secretKeyVariable)),
    );
  }
  ...
  public static async decrypt(
    ...
    const resolvedSecretKey =
      await CryptoEngineFacade.Environment.getSecretKeyFromEnvironment(
        secretKeyVariable,
      );
    CryptoEngineFacade.Validation.validateSecretKey(resolvedSecretKey);
    CryptoEngineFacade.Validation.validateInputs(
      encryptedData,
      resolvedSecretKey,
      "decrypt",
    );
    ...
    try {
      ...
      const { encryptionKey, hmacKey } =
        await CryptoEngineFacade.Argon2.deriveKeysWithArgon2(
          resolvedSecretKey,
          salt,
        );
      ...
      const decryptedBuffer =
        await CryptoEngineFacade.Decryption.performDecryption(
          iv,
          encryptionKey,
          cipherText,
        );
      ...
      ErrorHandler.captureError(
        error,
        "decrypt",
        "Failed to decrypt with AES-GCM.",
      );
      throw error;
    }
  }
  ...
  public static async decryptMultiple(
    ...
    if (!Array.isArray(encryptedValues)) {
      ErrorHandler.logAndThrow(
        "decryptMultiple",
        "encryptedValues must be an array",
      );
    }
