...
export default class CryptoDecryption {
  ...
  private static validateEncryptedComponents(
    ...
    if (!CryptoValidation.isValidBase64(cipherText))
      invalidComponents.push("cipherText");
    if (!CryptoValidation.isValidBase64(receivedHmac))
      invalidComponents.push("hmac");
  ...
  public static parseEncryptedData(encryptedData: string): {
    ...
    const encryptedPart = encryptedData.substring(
    ...
    if (!version || !salt || !iv || !cipherText || !receivedHmac) {
      ErrorHandler.logAndThrow(
        "CryptoEngine.parseEncryptedData",
  ...
  public static async performDecryption(
    ...
    return await CryptoWebOperations.decryptBuffer(
      ivBuffer,
      encryptionKey,
      cipherBuffer,
    );
