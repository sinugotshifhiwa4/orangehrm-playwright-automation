...
export default class CryptoArgon2 {
    ...
    try {
      ...
      const options: argon2.Options = {
        ...
        hashLength:
          ...
          CRYPTO_CONFIG.BYTE_LENGTHS.HMAC_KEY_LENGTH,
        salt: saltBuffer,
      ...
      const hmacKeyBuffer = derivedKeyBuffer.subarray(
        CRYPTO_CONFIG.BYTE_LENGTHS.SECRET_KEY,
      );
      ...
      const hmacKey = await CryptoWebOperations.importKeyForHMAC(
        Buffer.from(hmacKeyBuffer),
      );
   ...
   */
  private static async argon2Hashing(
    secretKey: string,
