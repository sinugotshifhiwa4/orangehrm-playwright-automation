...
export default class CryptoEncryption {
  ...
  public static async validateEncryptionPrerequisites(
    ...
    const actualSecretKey =
      await CryptoEnvironment.getSecretKeyFromEnvironment(secretKeyVariable);
  ...
  public static async generateEncryptionComponents(secretKey: string): Promise<{
    ...
    const { encryptionKey, hmacKey } = await CryptoArgon2.deriveKeysWithArgon2(
      secretKey,
      salt,
    );
  ...
  public static async createEncryptedPayload(
    ...
    // Encrypt the value
    const encryptedBuffer = await CryptoWebOperations.encryptBuffer(
      iv,
      encryptionKey,
      value,
    );
    const cipherText = Buffer.from(encryptedBuffer).toString(
      FileEncoding.BASE64,
    );
    ...
    const hmacBase64 = await CryptoWebOperations.computeHMAC(
      hmacKey,
      dataToHmac,
    );

    const formatted = this.formatEncryptedPayload(
      salt,
      ivBase64,
      cipherText,
      hmacBase64,
