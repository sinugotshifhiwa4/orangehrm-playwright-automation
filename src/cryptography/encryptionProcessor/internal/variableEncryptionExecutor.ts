...
import StagesFileManager from "../../../configuration/environment/manager/stagesFileManager.js";
...
export class VariableEncryptionExecutor {
  ...
  public async encryptVariables(
    ...
    try {
      const encryptedValues = await this.encryptAllValues(
        variablesToEncrypt,
        secretKeyVariable,
      );
      const updatedLines = this.applyEncryptedValues(
        envFileLines,
        encryptedValues,
      );
  ...
  private async encryptAllValues(
    ...
    for (const [key, value] of Object.entries(variables)) {
      try {
        ...
        encrypted[key] = await CryptoService.encrypt(
          trimmedValue,
          secretKeyVariable,
        );
  ...
  private applyEncryptedValues(
    ...
    return StagesFileManager.updateMultipleEnvironmentVariables(
      fileLines,
      encryptedValues,
    );
  }
