...
export default class SecureKeyGenerator {
  ...
  private static readonly WEB_CRYPTO_IV_LENGTH =
    CRYPTO_CONFIG.BYTE_LENGTHS.WEB_CRYPTO_IV;
  private static readonly SALT_LENGTH = CRYPTO_CONFIG.BYTE_LENGTHS.SALT;
  private static readonly SECRET_KEY_LENGTH =
    CRYPTO_CONFIG.BYTE_LENGTHS.SECRET_KEY;
  private static readonly MIN_VALIDATION_LIMIT =
   ...
   * @throws {Error} If the length is invalid or IV generation fails.
   */
  public static generateWebCryptoIV(
    length: number = this.WEB_CRYPTO_IV_LENGTH,
  ): Uint8Array {
   ...
   */
  public static generateBase64SecretKey(
    length: number = this.SECRET_KEY_LENGTH,
  ): string {
    return this.generate({
   ...
   * @throws {Error} If the length is invalid or key generation fails.
   */
  public static generateBufferSecretKey(
    length: number = this.SECRET_KEY_LENGTH,
  ): Buffer {
    return this.generate({
      type: CRYPTO_TYPE.SECRET_KEY,
   ...
   * @throws {Error} If the length is invalid or generation fails
   */
  private static generateSecureBytes(
    length: number,
    methodName: string,
  ): Buffer {
  ...
  private static validateLength(length: number, methodName: string): void {
    if (!Number.isInteger(length)) {
      ErrorHandler.logAndThrow(
        methodName,
        `Length must be an integer, got ${length}`,
      );
    }
