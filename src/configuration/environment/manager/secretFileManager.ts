...
export default class SecretFileManager {
  ...
  public static async storeKeyInFile(
    ...
    return this.executeWithFileLock(filePath, async () => {
      try {
        ...
        if (
          this.shouldSkipExistingKey(fileContent, keyName, options.skipIfExists)
        ) {
          return false;
        }

        const updatedContent = this.updateEnvironmentKey(
          fileContent,
          keyName,
          value,
        );
        await this.writeUpdatedEnvironmentFile(
          filePath,
          updatedContent,
          keyName,
        );
  ...
  public static async verifySecretKeyExists(
    ...
    try {
      ...
      const secretKeyValue = await this.getKeyValue(
        targetFilePath,
        secretKeyName,
      );
      ...
      const keyExists =
        secretKeyValue !== undefined && secretKeyValue.trim() !== "";

      if (keyExists) {
        logger.info(`Secret key "${secretKeyName}" verified successfully`);
      } else {
        logger.warn(
          `Secret key "${secretKeyName}" not found or empty in environment file`,
        );
      }
  ...
  public static async ensureEnvFileExists(filePath: string): Promise<void> {
    ...
    if (!fileExists) {
      logger.warn(
        `Environment file not found at "${filePath}". Creating new empty file.`,
      );
  ...
  public static handleMissingEnvFile(filePath: string): void {
    const isEncryptionEnabled = (process.env.PLAYWRIGHT_GREP || "").includes(
      "@full-encryption",
    );
   ...
   */
  private static createFileLock<T>(
    filePath: string,
    operation: () => Promise<T>,
  ): Promise<T> {
   ...
   */
  private static updateEnvironmentKey(
    fileContent: string,
    keyName: string,
    value: string,
  ): string {
   ...
   */
  private static environmentKeyExists(
    fileContent: string,
    keyName: string,
  ): boolean {
   ...
   */
  private static formatEnvironmentKeyValue(
    keyName: string,
    value: string,
  ): string {
   ...
   */
  private static addEnvironmentKey(
    fileContent: string,
    keyName: string,
    value: string,
  ): string {
