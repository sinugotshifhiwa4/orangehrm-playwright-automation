...
export default class StagesFileManager {
   ...
   */
  public static async readEnvironmentFileAsLines(
    filePath: string,
  ): Promise<string[]> {
    try {
   ...
   */
  public static async readEnvironmentVariables(
    filePath: string,
  ...
  public static async updateEnvironmentVariables(
    ...
    const lines = await this.readEnvironmentFileAsLines(filePath);
    const updatedLines = this.updateMultipleEnvironmentVariables(
      lines,
      variables,
    );

    // Pass the actual count of variables that were processed/encrypted
    await this.writeEnvironmentFileLines(
      filePath,
      updatedLines,
   ...
   */
  public static async doesEnvironmentFileExist(
    filePath: string,
  ): Promise<boolean> {
   ...
   */
  public static logEnvironmentFileNotFound(
    filePath: string,
   ...
   */
  public static parseFileContentToLines(
    content: string,
    filePath?: string,
  ): string[] {
    if (!content) {
   ...
   */
  public static extractEnvironmentVariables(
    lines: string[],
    ...
    for (const line of lines) {
      ...
      if (parsedVariable) {
        ...
        if (Object.prototype.hasOwnProperty.call(variables, key)) {
          logger.warn(
            `Duplicate environment variable '${key}' found at line ${lineNumber}`,
          );
        }
   ...
   */
  public static parseEnvironmentLine(
    line: string,
    lineNumber?: number,
  ): [string, string] | null {
    const trimmedLine = line.trim();
    ...
    if (!key || !CRYPTO_CONSTANTS.VALIDATION.ENV_VAR_KEY_PATTERN.test(key)) {
      ...
      logger.warn(
        `Invalid environment variable key format: '${key}'${lineInfo}`,
      );
      return null;
    }
